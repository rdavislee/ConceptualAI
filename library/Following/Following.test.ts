...
Deno.test({
  name: "Principle: user follows then unfollows another user",
  sanitizeOps: false,
  ...
  fn: async () => {
    ...
    try {
      ...
      const isFollowingArr = await following._isFollowing({
        follower: userA,
        followed: userB,
      });
      ...
      const afterUnfollow = await following._isFollowing({
        follower: userA,
        followed: userB,
      });
...
Deno.test({
  name: "Query: _followers reflects number of followers",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const [db, client] = await testDb();
...
Deno.test({
  name: "Query: _getFollowerCount returns correct count",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const [db, client] = await testDb();
...
Deno.test({
  name: "Lifecycle: deleteByFollower removes all follows by user",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const [db, client] = await testDb();
    ...
    try {
      await following.follow({ follower: userA, followed: userB });
      await following.follow({ follower: userA, followed: userC });
      const before = await following._following({ follower: userA });
      assertEquals(before[0].users.length, 2);

      await following.deleteByFollower({ follower: userA });

      const after = await following._following({ follower: userA });
      assertEquals(after[0].users.length, 0);
      const followersB = await following._followers({ followed: userB });
      assertEquals(followersB[0].users.includes(userA), false);
    } finally {
      await client.close();
    }
  },
...
Deno.test({
  name: "Lifecycle: deleteByFollowed removes all followers of user",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const [db, client] = await testDb();
    ...
    try {
      await following.follow({ follower: userA, followed: userB });
      await following.follow({ follower: userC, followed: userB });
      const before = await following._followers({ followed: userB });
      assertEquals(before[0].users.length, 2);

      await following.deleteByFollowed({ followed: userB });

      const after = await following._followers({ followed: userB });
      assertEquals(after[0].users.length, 0);
      const followingA = await following._following({ follower: userA });
      assertEquals(followingA[0].users.includes(userB), false);
    } finally {
      await client.close();
    }
  },
