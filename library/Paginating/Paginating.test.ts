...
Deno.test({
  name: "Actions: setPageSize and setEntryScore affect retrieval order",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const [db, client] = await testDb();
    ...
    try {
      ...
      const resizeRes = await paginating.setPageSize({
        bound: userBoundA,
        itemType: "myPosts",
        pageSize: 1,
      });
      assertEquals("ok" in resizeRes, true);
    ...
    try {
      ...
      for (let i = 1; i <= n; i++) {
        await paginating.upsertEntry({
          ...
          pageSize: i === 1 ? pageSize : undefined,
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const badPageSizeOnUpsert = await paginating.upsertEntry({
        bound: userBoundB,
        itemType: "myPosts",
        item: itemB,
        createdAt: t2,
      ...
      assertEquals("error" in badPageSizeOnUpsert, true);
      ...
      const badResize = await paginating.setPageSize({
        bound: userBoundB,
        itemType: "myPosts",
        pageSize: -5,
      });
