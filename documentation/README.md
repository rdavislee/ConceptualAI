...
- **Timeout note:** sandboxed generation phases have a 2-hour max; if one exceeds that, treat it as broken and resume/retry from last completed state.
...
- If your generated app uses AI, configure the backend `.env` with `AI_PROVIDER`, `AI_MODEL`, and the matching provider API key.
...
- [Deploy Your Generated App with Railway](./deploy-with-railway.md)
