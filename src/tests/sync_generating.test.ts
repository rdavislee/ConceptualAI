  ...
  fn: async () => {
    ...
    try {
      ...
      const appDescription =
        "A simple social media app with user profiles, posting, commenting, and liking for authenticated users.";
      ...
      if (project.status === "planning") {
        ...
        if (existingPlan) {
          ...
          if (existingPlan.description !== appDescription) {
            console.log(
              "Plan description mismatch. Deleting old plan/design/impl to restart...",
            );
            ...
            // Now initiate
            const planResult = await Planning.initiate({
              project: projectId,
              description: appDescription,
            });
      ...
      if (project.status === "planning_complete") {
        ...
        const existingDesign = await ConceptDesigning.designs.findOne({
          _id: projectId,
        });
        if (existingDesign) {
          console.log(
            "Design found but status was 'planning_complete'. advancing status...",
          );
          await ProjectLedger.updateStatus({
            project: projectId,
          ...
          const designResult = await ConceptDesigning.design({
            project: projectId,
            plan,
          });
      ...
      if (project.status === "design_complete") {
        ...
        const existingImpl = await Implementing.implJobs.findOne({
          _id: projectId,
        });
        if (existingImpl) {
          console.log(
            "Implementation found but status was 'design_complete'. advancing status...",
          );
          await ProjectLedger.updateStatus({
            project: projectId,
            status: "implemented",
          ...
          // Get design
          const designs = await ConceptDesigning._getDesign({
            project: projectId,
          });
          const design = designs[0].design;

          const implResult = await Implementing.implementAll({
            project: projectId,
            design,
          });
      ...
      if (
        project.status === "sync_generating" || project.status === "building" ||
        project.status === "assembled"
      ) {
        console.log(
          `Project found in ${project.status} state. Resetting to implemented for retry.`,
        );
        await ProjectLedger.updateStatus({
          project: projectId,
          status: "implemented",
      ...
      if (
        project.status === "implemented" || project.status === "syncs_generated"
      ) {
        ...
        console.log(
          "Waiting for Sync Generation agent (this may take a minute)...",
        );
        ...
        console.log(
          `Generated ${response.endpointBundles.length} endpoint bundles.`,
        );

        // Verify Project Status Update
        const finalProj = await ProjectLedger.projects.findOne({
          _id: projectId,
        });
        ...
        const getResponseArray = await Requesting._awaitResponse({
          request: getReq,
        });
        ...
        assertEquals(
          getResponse.endpointBundles.length,
          response.endpointBundles.length,
        );
      } else {
